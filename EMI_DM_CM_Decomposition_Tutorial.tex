...
\begin{circuitikz}[american, scale=0.88, transform shape]
  ...
  \draw (0,0) node[ocirc]{}
        ...
        to[L, l_=$L$] (5.0,0)
        -- (6,0);
  \draw[->>, thick] (0.25,-0.42) -- (1.2,-0.42)
        node[midway, below, font=\small]{$i_2$};

  %--- Shunt capacitors (each line to its own ground) ---
  \draw (6,2) to[C, l=$C$]  (6, 0.6) node[ground]{};
  \draw (6,0) to[C, l=$C$]  (6,-1.2) node[ground]{};

  %--- Floating reference P (below both lines) ---
  \coordinate (Pnd) at (-1.4,-0.9);
  \fill[black] (Pnd) circle (2.8pt);
  \node[left, font=\small\bfseries] at (Pnd) {$P$};

  %--- v1P: dashed from line 1 terminal down to P ---
  \draw[dashed, thin] (0,2) -- (Pnd)
        node[midway, right, font=\scriptsize]{$v_{1P}$};

  %--- v2P: dashed from line 2 terminal down to P ---
  \draw[dashed, thin] (0,0) -- (Pnd)
        node[midway, right, font=\scriptsize]{$v_{2P}$};

  %--- v_Pg: from P to ground ---
  \draw (Pnd) -- (-1.4,-1.8) node[ground]{};
  \node[right, font=\scriptsize] at (-1.4,-1.35) {$v_{Pg}$};
...
\caption{Circuit for $N = 2$ lines.
  The column vectors that enter the $2{\times}2$ transformation are
  $\ivec  = [i_1,\;i_2]^T$ (line currents, left-to-right)
  and $\vvec = [v_{1P},\;v_{2P}]^T$
  (line-to-floating-reference voltages).
  $P$ sits below both lines; $v_{Pg}$ is the only path from~$P$ to ground.}
\label{fig:ckt_N2}
\end{figure}
...
\begin{circuitikz}[american, scale=0.88, transform shape]
  ...
  %--- Shunt capacitors ---
  \draw (6,4) to[C, l=$C$]   (6, 2.7) node[ground]{};
  \draw (6,2) to[C, l=$C$]   (6, 0.7) node[ground]{};
  \draw (6,0) to[C, l=$kC$]  (6,-1.3) node[ground]{};

  %--- Floating reference P (below all lines) ---
  \coordinate (Pnd) at (-1.4,-0.9);
  \fill[black] (Pnd) circle (2.8pt);
  \node[left, font=\small\bfseries] at (Pnd) {$P$};

  %--- Voltage dashed lines ---
  \draw[dashed, thin] (0,4) -- (Pnd)
        node[midway, right, font=\scriptsize]{$v_{1P}$};
  \draw[dashed, thin] (0,2) -- (Pnd)
        node[midway, right, font=\scriptsize]{$v_{2P}$};
  \draw[dashed, thin] (0,0) -- (Pnd)
        node[midway, right, font=\scriptsize]{$v_{3P}$};
  ...
  \node[below, font=\footnotesize, color=gray!70!black] at (0,0) {Line 3 (asymmetric, $kC$)};

\end{circuitikz}
\caption{Circuit for $N = 3$ lines.
  Line~3 is asymmetric: its shunt capacitance is $kC$ with $k < 1$,
  while lines~1 and 2 each have capacitance~$C$.
  The input vectors are
  $\ivec = [i_1,\;i_2,\;i_3]^T$ and
  $\vvec = [v_{1P},\;v_{2P},\;v_{3P}]^T$.}
\label{fig:ckt_N3}
\end{figure}
