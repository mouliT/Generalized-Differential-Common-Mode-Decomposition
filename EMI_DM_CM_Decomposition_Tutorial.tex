...
\textbf{The normalisation factor $\Ni$:}
The paper uses $\Ni = 2$.  This specific choice ensures that for
\emph{symmetric} lines the DM impedances in the decomposed circuit
equal the \emph{series combination} of the two per-line impedances
seen by that DM current loop — a physically intuitive result
(two resistors $R$ in series give $2R$, which equals $R' = \Ni R$
with $\Ni = 2$).  An alternative $\Ni = \sqrt{N}$ would give
$|\mathbf{T}_N^i| = 1$ (unit determinant) but produces less
intuitive element values.
