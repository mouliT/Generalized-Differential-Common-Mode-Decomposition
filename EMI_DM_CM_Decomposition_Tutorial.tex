...
\begin{physbox}[EMI-Frequency Approximation: $i_n' \approx 0$]
Look again at Fig.~\ref{fig:fig1}.  Each line has \emph{two} currents:
$i_n$ flowing from the source through $R$, $L$, and $C_n$, and
$i_n'$ flowing into the load.  By KCL at the top plate of $C_n$:
\[
    i_{C_n} = i_n - i_n'.
\]
The \emph{exact} KVL for line $n$ is therefore
\begin{equation}
    v_{nP} + v_{Pg}
...
At \textbf{EMI frequencies} (tens of kHz and above), the parasitic
capacitor $C_n$ is a near-short circuit:
\[
    \frac{1}{\omega C_n} \ll Z_{\mathrm{load}}.
\]
Almost all high-frequency current takes the low-impedance cap-to-ground
path, leaving negligible current for the load:
\[
    i_n' \approx 0 \quad \Longrightarrow \quad i_{C_n} \approx i_n.
\]
Substituting into Eq.~\eqref{eq:KVL_exact} recovers the compact form
\[
    v_{nP} + v_{Pg}
    = \Bigl(R + pL + \tfrac{S_n}{p}\Bigr)\,i_n,
\]
which is the equation the paper uses throughout.  \textbf{Every
equation that follows assumes this approximation.}
\end{physbox}

Applying this approximation to each of the $N$ lines:
\begin{align*}
    v_{1P} + v_{Pg}
