...
Formally, if $A$ represents a linear map in the standard basis and
$P$ is the change-of-basis matrix (columns = new basis vectors), then
\[
    A_{\mathrm{new}} = P^{-1} A P
\]
represents the \emph{same} map in the new basis.

The mixed-mode equation is exactly this:
\[
    \underbrace{\mathbf{Z}}_{\text{per-line basis}}\,\mathbf{i}
    = \mathbf{v}_{nP} + v_{Pg}\,\mathbf{1}_N
    \qquad\xrightarrow{\;\text{change basis}\;}\qquad
    \underbrace{\mathbf{Z}_{\mathrm{DCM}}}_{\text{DM--CM basis}}\,\iDCM
    = \vDCM + v_{Pg}\,\mathbf{e}_N.
\]
$\mathbf{Z}_{\mathrm{DCM}} = \TvN\,\mathbf{Z}\,(\TiN)^{-1}$ is the
same impedance operator — the same physics — expressed in the DM--CM
coordinate system.
...
In Strang's abstract change-of-basis story, the \emph{same} matrix
$P$ transforms both the vector $\mathbf{x}$ and the image
$A\mathbf{x}$, giving $P^{-1}AP$.  So why does the circuit require
two \emph{different} matrices — $\TiN$ for currents and $\TvN$ for
voltages?

Because voltages and currents are governed by \emph{different physical
laws} that produce different natural operations:
